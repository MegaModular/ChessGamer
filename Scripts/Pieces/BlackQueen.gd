...
func update_areas():
	if infotransfer.selected == self and infotransfer.turn == "black":
		...
		for i in $TopRight.get_children():
			i.force_raycast_update()
			if i.is_colliding():
				if i.get_collider().is_in_group("WhitePiece") and total_collisions_A == 0:
		...
		for i in $BottomRight.get_children():
			i.force_raycast_update()
			if i.is_colliding():
				if i.get_collider().is_in_group("WhitePiece") and total_collisions_B == 0:
		...
		for i in $BottomLeft.get_children():
			i.force_raycast_update()
			if i.is_colliding():
				if i.get_collider().is_in_group("WhitePiece") and total_collisions_C == 0:
		...
		for i in $TopLeft.get_children():
			i.force_raycast_update()
			if i.is_colliding():
				if i.get_collider().is_in_group("WhitePiece") and total_collisions_D == 0:
